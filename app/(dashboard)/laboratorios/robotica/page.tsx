...
const LAB_NAME = "Laboratório de Robótica" as const

export default function LabRoboticaPage() {
  const labItems = useMemo(() => mockItems.filter((i) => i.laboratory === LAB_NAME), [])
  const labMaintenances = useMemo(() => mockMaintenances.filter((m) => m.itemCode.startsWith("LAB-ROB") || m.itemCode.startsWith("LAB-FAB")), [])
    ...
    <>
      ...
      <div className="flex-1 space-y-6 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Laboratório de Robótica</h1>
            <p className="text-muted-foreground">
              Visão geral dos equipamentos e atividades do laboratório
            </p>
          </div>
          <Button asChild>
            <Link href="/inventario?lab=robotica">
